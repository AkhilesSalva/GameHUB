...
// Fungsi untuk mendapatkan genre game (untuk card) - Diambil dari index.php
...
// Catatan: Karena fitur download/koleksi user belum diimplementasi, 
// query ini HANYA menampilkan game teratas sebagai placeholder.
// DI MASA DEPAN, ganti query ini dengan JOIN ke tabel 'user_koleksi'
$my_collection_query = "SELECT g.id, g.nama, g.gambar_path, g.download_count, COUNT(k.id) as komentar_count 
                        FROM games g 
                        LEFT JOIN komentar k ON g.id = k.game_id
                        GROUP BY g.id 
                        ORDER BY g.download_count DESC LIMIT 6"; 
$my_collection_result = mysqli_query($koneksi, $my_collection_query);
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Koleksi Saya - Game Hub</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        /* ========================================================================= */
        /* 3. PAGE CONTENT & CARDS (Sama dengan semua_game.php) */
        /* ========================================================================= */
        .page-header { margin-bottom: 25px; }
        .page-header h1 { color: white; font-size: 2.2em; }
        .page-header h1 span { color: #4CAF50; }
        
        .info-box { background-color: #1e293b; padding: 20px; border-radius: 8px; margin-bottom: 30px; }
...
<body>
        ...
        <div class="navbar-content">
            ...
            <div class="nav-right">
                <div class="nav-links">
                    ...
                    <?php if ($is_logged_in): ?>
                        <a href="<?php echo BASE_URL; ?>/koleksiku.php" class="active-link">Koleksiku</a>
                    <?php endif; ?>
                </div>
    ...
    <div class="store-container">
        <div class="page-header">
            ...
            <p style="color: #b0b0b0; margin-top: 10px;">
                Selamat datang di koleksi game Anda, **<?php echo htmlspecialchars($username_display); ?>**.
            </p>
        </div>

        <div class="info-box">
             <p>Saat ini, fitur menyimpan game ke koleksi masih dalam pengembangan. Berikut adalah beberapa game populer.</p>
        </div>
        
        <div class="game-grid">
            <?php if ($my_collection_result && mysqli_num_rows($my_collection_result) > 0): ?>
                <?php while ($game = mysqli_fetch_assoc($my_collection_result)): ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <p class="empty-state">Anda belum memiliki game di koleksi. Mulai jelajahi dan download sekarang!</p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        // JS untuk fungsionalitas front-end jika diperlukan (saat ini kosong)
    </script>
</body>
</html>